    ...
    <section id="skills" className="bg-slate-50 py-16 sm:py-20">
      <div className="container mx-auto px-4 md:px-6">
        <div className="mb-12 text-center sm:mb-16">
          ...
          <h2 className="mb-4 text-3xl font-bold tracking-tight text-slate-950 md:text-4xl">Skills and expertise</h2>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {skillCategories.map((category) => (
            <div key={category.title} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <h3 className="mb-4 text-xl font-semibold text-slate-950">{category.title}</h3>
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill) => (
                  <span key={skill} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-700 sm:text-sm">
                    {skill}
                  </span>
                ))}
              </div>
